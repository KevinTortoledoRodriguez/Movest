...
export class Aprobadas extends Component {
    constructor(){
        super();
        this.state = {
            solicitudes:[],
    ...
    filtraSolicitudes(alumnos){
        ...
        solicitudes.map((soli) => {
            if(user.puesto === 'Jefe de Departamento'){
                if(soli.status === 'Aprobada' && soli.institucionOrigen === user.institucion){
                    solicitudesFiltradas.push(soli)
                }
            } else {
                if(soli.status === 'Aprobada' && soli.institucionOrigen === user.institucion && alumnos.includes(soli.alumno) ){
                    solicitudesFiltradas.push(soli)
                }
            }
        })
